...
def trainBatch(crnn, criterion, optimizer, train_iter, converter, use_dctc=False):
    ...
    if use_dctc:
        ...
        cost = criterion(preds, text, predsSize, length)
    crnn.zero_grad()
    cost.backward()
    optimizer.step()
    return cost

def val(net, test_dataset, criterion, epoch, args, converter, use_dctc=False):
    global max_acc
    ...
    for i in range(max_iter):
        ...
        if use_dctc:
            ...
            preds = torch.log_softmax(preds, 2)
            cost = criterion(preds, texts, preds_size, lengths)
        loss.add(cost)
        _, preds = preds.max(2)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--epoch', type=int, default=125, help='')
    ...
    parser.add_argument('--dctc', default=False, action='store_true')
    ...
    if not args.test:
        train_dataloader = torch.utils.data.DataLoader(
            train_dataset, batch_size=args.batch_size, num_workers=8, shuffle=True, drop_last=False,
    ...
    for epoch in range(args.epoch):
        ...
        while i < len(train_dataloader):
            ...
            crnn.train()
            cost = trainBatch(crnn, criterion, optimizer, train_iter, converter, use_dctc=True)
            loss_avg.add(cost)
            i += 1
            iters = len(train_dataloader) * epoch + i
            if iters % 10 == 0:
                print('epoch:%d train:%d/%d' % (epoch, i, len(train_dataloader)), cost.item())
        val(crnn, test_dataset, criterion, epoch, args, converter, use_dctc=True)
        # # scheduler
